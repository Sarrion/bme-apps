...
def checklist(ide, options):
    return dcc.Checklist(id=ide, options = options, value=[options[0]['value']], labelStyle={'display':'inline-block'})
...
def rgslider(ide, var, n_marks):
    ...
    return dcc.RangeSlider(id = ide, min=limits[0], max=limits[1], 
                           ...
                           marks = {i:'{}'.format(i) for i in linspace(limits[0], limits[1], n_marks)})
...
layout = {
    ...
    #"paper_bgcolor": "rgba(220, 220, 220, 0.8)",
        ...
        Col([
            html.Div([
                html.Div([
                    ...
                    Row([Col(html.H5('Q6: '), sm=1), Col(rgslider('rgslider_Q6_1',df.Q6, 3))]),
                    Row([Col(html.H5('Q7: '), sm=1), Col(checklist('multi_Q7_1',dropdown_options.Q7))]),
                    Row([Col(html.H5('Q8: '), sm=1), Col(rgslider('rgslider_Q8_1',df.Q8, 3))]),
                    Row([Col(html.H5('Q9: '), sm=1), Col(checklist('multi_Q9_1',dropdown_options.Q9))]),
                    Row([Col(html.H5('Q10: '), sm=1), Col(rgslider('rgslider_Q10_1',df.Q10, 3))])
                ], style={'width':'95%'})
        ...
        Col([
            html.Div([
                html.Div([
                    ...
                    Row([Col(html.H5('Q6: '), sm=1), Col(rgslider('rgslider_Q6_2',df.Q6, 3))]),
                    Row([Col(html.H5('Q7: '), sm=1), Col(checklist('multi_Q7_2',dropdown_options.Q7))]),
                    Row([Col(html.H5('Q8: '), sm=1), Col(rgslider('rgslider_Q8_2',df.Q8, 3))]),
                    Row([Col(html.H5('Q9: '), sm=1), Col(checklist('multi_Q9_2',dropdown_options.Q9))]),
                    Row([Col(html.H5('Q10: '), sm=1), Col(rgslider('rgslider_Q10_2',df.Q10, 3))])
                ], style={'width':'95%'})
            ], style={'width':'95%','box-shadow':'2px 2px 2px grey'})
        ])
    ]),
...
], style = {'background-color': 'rgba(1, 1, 1, 0.1)'})
